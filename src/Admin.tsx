...
import { collection, addDoc, getDocs, deleteDoc, doc, onSnapshot, query, orderBy, updateDoc, increment, setDoc, getDoc } from 'firebase/firestore';
      ...
      <main className="flex-1 overflow-y-auto p-10 space-y-10">
        <header className="flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-500 text-sm font-medium">Manage your IPTV ecosystem</p>
          </div>
          <button 
            onClick={handleCheckChannels}
            disabled={isChecking}
            className={`px-6 py-3 rounded-2xl font-black text-xs tracking-widest flex items-center gap-3 transition-all ${isChecking ? 'bg-gray-800 text-gray-500' : 'bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-600/20'}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          {[
            ...
            <div key={i} className="bg-[#121214] p-6 rounded-[24px] border border-white/5 shadow-xl">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
          {/* Channel Management */}
          <section className="bg-[#121214] p-8 rounded-[32px] border border-white/5 shadow-2xl">
            <h3 className="text-xl font-black mb-6 flex items-center gap-3">
              <span className="w-1.5 h-6 bg-[#ff003c] rounded-full"></span>
              CHANNELS
            </h3>
            <form onSubmit={handleAddChannel} className="grid grid-cols-2 gap-4 mb-8">
              ...
              <input name="url" placeholder="Stream URL" className="col-span-2 bg-white/5 border border-white/5 rounded-xl p-3 text-sm outline-none focus:border-[#ff003c]/50" required />
              ...
              <button className="col-span-2 bg-[#ff003c] py-3 rounded-xl font-black text-xs tracking-widest shadow-lg shadow-[#ff003c]/20">ADD CONTENT</button>
            </form>
            
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 no-scrollbar">
              {channels.map(ch => (
                <div key={ch.id} className="flex items-center justify-between bg-white/5 p-4 rounded-2xl border border-white/5 group hover:bg-white/10 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-sm font-black group-hover:text-[#ff003c] transition-colors">{ch.name}</div>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 bg-black/30 px-2 py-0.5 rounded-md">{ch.type || 'live'}</span>
                        <span className={`text-[9px] font-black uppercase tracking-widest ${ch.status === 'online' ? 'text-emerald-500' : 'text-red-500'}`}>
                          {ch.status || 'online'}
                        </span>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => handleDeleteChannel(ch.id!)} className="w-10 h-10 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all">
          ...
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                {sliderAds.map(ad => (
                  <div key={ad.id} className="relative group rounded-2xl overflow-hidden border border-white/5">
                    <img src={ad.image} className="w-full h-24 object-cover" alt="" />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                      <button onClick={() => handleDeleteSliderAd(ad.id!)} className="w-10 h-10 rounded-full bg-red-500 text-white"><i className="fa-solid fa-trash"></i></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-[#121214] p-8 rounded-[32px] border border-white/5 shadow-2xl">
              ...
              <div className="grid grid-cols-3 gap-4">
                {gridAds.map(ad => (
                  <div key={ad.id} className="relative group rounded-2xl overflow-hidden border border-white/5">
                    <img src={ad.image} className="w-full h-20 object-cover" alt="" />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                      <button onClick={() => handleDeleteGridAd(ad.id!)} className="w-10 h-10 rounded-full bg-red-500 text-white"><i className="fa-solid fa-trash"></i></button>
                    </div>
                  </div>
                ))}
              </div>
