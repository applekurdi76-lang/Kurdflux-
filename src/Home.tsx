    ...
    <div className="min-h-screen bg-[#050505] text-white flex overflow-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block w-80">
              ...
              <input 
                ...
                className={`w-full bg-white/5 border border-white/5 rounded-2xl py-2.5 text-sm outline-none focus:border-[#ff003c]/50 transition-all ${isRtl ? 'pr-12 pl-4' : 'pl-12 pr-4'}`}
                value={searchQuery}
      ...
      <AnimatePresence>
        {currentPlayer && (
          ...
          >
            {/* Player Header */}
            <div className={`absolute top-0 inset-x-0 p-6 bg-gradient-to-b from-black/90 to-transparent z-[2100] flex justify-between items-center transition-opacity duration-500 ${!showUI ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
              <div className="flex items-center gap-4">
                <button onClick={() => setCurrentPlayer(null)} className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center hover:bg-[#ff003c] transition-all">
                  <i className={`fa-solid ${isRtl ? 'fa-chevron-right' : 'fa-chevron-left'} text-lg`}></i>
                </button>
                <div>
                  <h4 className="text-xl font-black tracking-tight">{currentPlayer.name}</h4>
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl text-[10px] font-black border transition-all ${isTurboMode ? 'bg-green-500 border-green-500 text-black' : 'bg-white/5 border-white/10 text-white'}`}
                >
                  TURBO {isTurboMode ? 'ACTIVE' : 'OFF'}
                </button>
              </div>
            </div>
